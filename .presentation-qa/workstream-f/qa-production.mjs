...
const evidenceDir = path.join(root, '.presentation-qa', 'workstream-f', 'production')
...
async function runAdmin(browserType, name) {
  ...
  try {
    ...
    const heading = await page.getByText(/clinic-scoped clinical intelligence/i).count()
    if (heading < 1) findings.push('reports description missing')
    if ((await page.getByText('Access denied').count()) > 0) findings.push('admin denied on reports')
    if ((await page.getByRole('button', { name: /acknowledge/i }).count()) > 0) {
      findings.push('acknowledge action present on reports')
    }
    if ((await page.getByRole('button', { name: /resolve/i }).count()) > 0) {
      findings.push('resolve action present on reports')
    }

    await page.screenshot({ path: path.join(evidenceDir, `${name}-reports-30d.png`), fullPage: true })

    const seven = page.getByRole('button', { name: '7 Days' })
    if ((await seven.count()) < 1) findings.push('7 day period chip missing')
    else {
      await seven.click()
      await page.waitForTimeout(1500)
      await page.screenshot({ path: path.join(evidenceDir, `${name}-reports-7d.png`), fullPage: true })
    }

    const ninety = page.getByRole('button', { name: '90 Days' })
    if ((await ninety.count()) < 1) findings.push('90 day period chip missing')
    else {
      await ninety.click()
      await page.waitForTimeout(1500)
      await page.screenshot({ path: path.join(evidenceDir, `${name}-reports-90d.png`), fullPage: true })
    }

    if ((await page.getByText('Last 12 months').count()) > 0) findings.push('12 month period still in launch UI')

    const exportBtn = page.getByRole('button', { name: /export csv/i })
    if ((await exportBtn.count()) < 1) findings.push('export csv missing')

    await page.setViewportSize({ width: 900, height: 900 })
    await page.waitForTimeout(400)
    await page.screenshot({ path: path.join(evidenceDir, `${name}-reports-900.png`), fullPage: true })
    await page.setViewportSize({ width: 1440, height: 900 })

    if (consoleErrors.some((msg) => /reports/i.test(msg) && !/favicon/i.test(msg))) {
      findings.push(`console errors: ${consoleErrors.slice(0, 3).join(' | ')}`)
    }
  } catch (error) {
...
if (failed.length) {
  console.error('workstream_f_browser_qa: FAIL')
  process.exit(1)
}
console.log('workstream_f_browser_qa: ok')
